...
class CgroupTest :
    FunSpec({
        ...
        context("setup") {
            // subtree_control propagation across all ancestors
            ...
            test("enables controllers at every ancestor for nested cgroupsPath") {
            ...
            test("enables controllers only at root for top-level cgroupsPath") {
                val fs = FakeFileSystem()
                CgroupV2(fs).setup(
                    pid = 42,
            ...
            test("short-circuits when both cgroupPath and resources are null") {
                val fs = FakeFileSystem()
            ...
            test("writes cpu.max as 'quota period' when both are set") {
                val fs = FakeFileSystem()
                CgroupV2(fs).setup(
                    pid = 1,
                    cgroupPath = "x",
            ...
            test("writes cpu.max as 'max' when quota is non-positive") {
                val fs = FakeFileSystem()
                CgroupV2(fs).setup(
                    pid = 1,
                    cgroupPath = "x",
            ...
            test("converts cgroup v1 shares to v2 cpu.weight") {
                val fs = FakeFileSystem()
            ...
            test("writes memory.swap.max as swap minus limit (cgroup v2 semantics)") {
                val fs = FakeFileSystem()
                CgroupV2(fs).setup(
                    pid = 1,
                    cgroupPath = "x",
        ...
        context("getPids") {
            test("returns the PIDs read from cgroup.procs") {
                val fs = FakeFileSystem()
            ...
            test("accepts a leading slash in the path") {
                val fs = FakeFileSystem()
            ...
            test("skips blank and non-numeric lines") {
                val fs = FakeFileSystem()
                fs.files["/sys/fs/cgroup/x/cgroup.procs"] = "1\n\nabc\n2\n"
        ...
        context("cleanup") {
            test("is a no-op for null path") {
                val fs = FakeFileSystem()
                CgroupV2(fs).cleanup(null)
                fs.calls.isEmpty() shouldBe true
            }

            test("removes the directory when present") {
                val fs = FakeFileSystem()
            ...
            test("tolerates missing directory") {
                val fs = FakeFileSystem()
